// Package k8s
// kube registry simply implements the Kubernetes-based Registry
package k8s
...
func NewRegistry(clientSet *kubernetes.Clientset) *Registry {
	opts := informers.WithNamespace(GetNamespace())
	informerFactory := informers.NewSharedInformerFactoryWithOptions(clientSet, time.Minute*5, opts)
...
func (s *Registry) GetService(ctx context.Context, name string) ([]*registry.ServiceInstance, error) {
	...
	for _, pod := range pods {
		...
		log.Debugw("namespace", pod.GetNamespace(), "instance", instance)
