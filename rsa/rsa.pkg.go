...
import (
	...
	"encoding/pem"
	"errors"
	base64pkg "github.com/eden/go-kratos-pkg/base64"
)
...
func NewRsaCipherBase64(pubKeyBase64, priKeyBase64 []byte) (*RsaCipher, error) {
	pubKey, err := base64pkg.Decode(pubKeyBase64)
	if err != nil {
		return nil, err
	}
	priKey, err := base64pkg.Decode(priKeyBase64)
	if err != nil {
		return nil, err
	}
...
func (r *RsaCipher) Encrypt(plainText []byte) ([]byte, error) {
	...
	return base64pkg.Encode(encrypted), nil
}

func (r *RsaCipher) Decrypt(cipherText []byte) ([]byte, error) {
	s, err := base64pkg.Decode(cipherText)
	if err != nil {
		return nil, err
	}
